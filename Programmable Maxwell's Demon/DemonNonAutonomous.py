...
rodVo = cylinder(pos=vector(0, -1.5 * H, -0.55 * R), radius=Dia_axle / 2, axis=vector(0, 3 * H, 0), color=color.orange)
rodVpi = cylinder(pos=vector(0, -1.5 * H, 0.55 * R), radius=Dia_axle / 2, axis=vector(0, 3 * H, 0), color=color.orange)
...
while True:
    ...
    for p in paddles:
        update = True
        if -H < p.pos.y < H:                 # neeed to introduce hexagonal nos, memory location track
            print(p)
            if 0 < p.theta < pi:
                rodVo.pos.y = 1.5 * H
                rodVpi.pos.y = - 1.5 * H
            elif pi < p.theta < 2 * pi:
                rodVpi.pos.y = 1.5 * H
                rodVo.pos.y = - 1.5 * H
            if p.omega != demon.omega:
                deltat = (true_angle(demon.theta) - p.theta) / (p.omega - demon.omega)
                if 0 <= deltat <= dt:  # there will be a collision  # need to improve collision condition
                    collision = True
